...
#URL of current month
year = time.strftime("%Y")
prev_mon = int(time.strftime("%m")) - 1
prev_mon_text = calendar.month_name[prev_mon]
prev_mon_abbr = prev_mon_text[0:3]
prev_mon_abbr = prev_mon_abbr.lower()
url = "http://www.un.org/en/peacekeeping/contributors/" + str(year) + "/" + prev_mon_abbr + year[2:4] + "_3.pdf"
...
##set current date
#parse URL for current month and year
urlTag = re.search("contributors/(\d\d\d\d)/(.*?_3.pdf)", url)
urlEnd = urlTag.group(2)
urlMonth = re.match("(...).*?(?:\d\d)?(\d\d)?_3.pdf", urlEnd)
year = int(urlTag.group(1))
m3 = ["jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec"]
month = m3.index(urlMonth.group(1).lower())+1
day = calendar.monthrange(year, month)[1]
dateString = str(month) + '/' + str(day) + '/' + str(year-2000)
date = datetime(year, month, day).strftime('%Y%m%d')


# tccReplace = ["Moldova, Republic of", "Tanzania, United Republic of", "Macedonia, FYROM","The former Yugoslav Republic of Macedoniaao"]
#ISO3 dict
